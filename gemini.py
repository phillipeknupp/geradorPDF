...
class PDFManagerApp(QMainWindow):
    ...
    def setup_tab_pages(self):
        ...
        btn_ocr = QPushButton("Fazer OCR / Extrair")
    ...
    # ======================================================================
    # NOVAS IMPLEMENTAÇÕES DE BOTÕES
    # ======================================================================

    def pdf_to_images(self):
        pages = self.get_selected_pages_from_thumbs()
        if not pages:
            return QMessageBox.warning(self, "Aviso", "Selecione as páginas primeiro.")

        out_dir = QFileDialog.getExistingDirectory(self, "Selecionar Pasta de Destino")
        if not out_dir: return

        try:
            for i, (path, p_num) in enumerate(pages):
                doc = fitz.open(path)
                page = doc.load_page(p_num)
                pix = page.get_pixmap(dpi=150)
                output_name = f"imagem_pag_{i + 1}.png"
                pix.save(os.path.join(out_dir, output_name))
                doc.close()
            QMessageBox.information(self, "Sucesso", "Páginas salvas como imagens com sucesso!")
        except Exception as e:
    ...
    def rotate_selected(self):
        ...
        angle, ok = QInputDialog.getInt(self, "Rotacionar", "Graus de rotação (ex: 90, 180, 270):", 90, 0, 360, 90)
        if not ok: return

        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF Rotacionado", "", "PDF Files (*.pdf)")
        if not out_path: return

        try:
            new_doc = fitz.open()
            for path, p_num in pages:
                src = fitz.open(path)
                new_doc.insert_pdf(src, from_page=p_num, to_page=p_num)
                src.close()
            ...
            new_doc.save(out_path)
            new_doc.close()
            QMessageBox.information(self, "Sucesso", f"PDF rotacionado salvo em:\n{out_path}")
        except Exception as e:
    ...
    def add_watermark_to_selected(self):
        ...
        text, ok = QInputDialog.getText(self, "Marca D'água", "Digite o texto da marca d'água:")
        if not ok or not text: return

        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF com Marca D'água", "", "PDF Files (*.pdf)")
        if not out_path: return
        ...
        try:
            ...
            for page in new_doc:
                rect = page.rect
                page.insert_text(fitz.Point(50, rect.height - 50), text, fontsize=40, color=(0.5, 0.5, 0.5), rotate=45)
    ...
    def apply_redaction(self):
        ...
        text, ok = QInputDialog.getText(self, "Remover Confidencial",
                                        "Digite o texto que deseja censurar (cobrir de preto):")
        if not ok or not text: return

        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF Censurado", "", "PDF Files (*.pdf)")
        if not out_path: return
        ...
        try:
            ...
            new_doc.save(out_path)
            new_doc.close()
            QMessageBox.information(self, "Sucesso", "Termos ocultados e PDF salvo com sucesso.")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao aplicar redação:\n{e}")
    ...
    def apply_ocr(self):
        ...
        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar Texto Extraído", "", "Text Files (*.txt)")
        if not out_path: return
        ...
        try:
            with open(out_path, "w", encoding="utf-8") as f:
                for path, p_num in pages:
                    ...
                    try:
                        # Tenta usar a engine de OCR nativa (requer tesseract no sistema operacional do usuário)
                        text = page.get_textpage_ocr(language="por").extractText()
                    except:
                        # Fallback robusto para extração de texto padrão
                        text = page.get_text("text")

                    f.write(f"--- Página {p_num + 1} do documento {os.path.basename(path)} ---\n")
                    f.write(text + "\n\n")
                    doc.close()
            QMessageBox.information(self, "Sucesso", f"Texto extraído salvo em:\n{out_path}")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao realizar OCR / Extração de Texto:\n{e}")

    def compress_pdfs(self):
        if not self.pdf_files:
            return QMessageBox.warning(self, "Aviso", "Nenhum PDF na lista. Adicione arquivos primeiro.")
        ...
        try:
            for path in self.pdf_files:
                doc = fitz.open(path)
                out_name = "Comprimido_" + os.path.basename(path)
                # Deflate e garbage são os métodos built-in do PyMuPDF para comprimir / limpar estrutura morta
                doc.save(os.path.join(out_dir, out_name), garbage=4, deflate=True)
                doc.close()
            QMessageBox.information(self, "Sucesso", "Os PDFs foram comprimidos na pasta selecionada.")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao comprimir PDFs:\n{e}")

    def images_to_pdf(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Selecionar Imagens", "", "Imagens (*.png *.jpg *.jpeg *.bmp)")
        if not files: return

        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF de Imagens", "", "PDF Files (*.pdf)")
        if not out_path: return
        ...
        try:
            ...
            doc.save(out_path)
            doc.close()
            QMessageBox.information(self, "Sucesso", "PDF criado a partir das imagens selecionadas!")
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao converter imagens para PDF:\n{e}")

    def unlock_pdf(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Selecionar PDF Bloqueado", "", "PDF Files (*.pdf)")
        if not file_path: return

        doc = fitz.open(file_path)
        if not doc.is_encrypted:
            doc.close()
            return QMessageBox.information(self, "Aviso", "O PDF selecionado não está bloqueado com senha.")

        pw, ok = QInputDialog.getText(self, "Desbloquear", "Digite a senha do PDF:", QLineEdit.Password)
        if not ok: return

        if doc.authenticate(pw):
            out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF Desbloqueado", "", "PDF Files (*.pdf)")
            if out_path:
                doc.save(out_path)
                QMessageBox.information(self, "Sucesso", "PDF desbloqueado e salvo com sucesso!")
        else:
            QMessageBox.warning(self, "Erro", "A senha inserida está incorreta.")

        doc.close()

    def lock_pdf(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Selecionar PDF para Proteger", "", "PDF Files (*.pdf)")
        if not file_path: return

        pw, ok = QInputDialog.getText(self, "Bloquear PDF", "Defina uma senha para o PDF:", QLineEdit.Password)
        if not ok or not pw: return

        out_path, _ = QFileDialog.getSaveFileName(self, "Salvar PDF Protegido", "", "PDF Files (*.pdf)")
        if not out_path: return

        try:
            doc = fitz.open(file_path)
            perm = fitz.PDF_PERM_ACCESSIBILITY | fitz.PDF_PERM_PRINT  # Permissões básicas
            doc.save(out_path, encryption=fitz.PDF_ENCRYPT_AES_256, owner_pw=pw, user_pw=pw, permissions=perm)
            doc.close()
            QMessageBox.information(self, "Sucesso", "PDF protegido com senha com sucesso!")
        except Exception as e:
