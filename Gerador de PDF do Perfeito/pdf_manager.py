...
# ======================================================================
# Utilitário de recursos – compatível com PyInstaller --onefile
# ======================================================================
def resource_path(relative_name):
    """
    Retorna o caminho absoluto para um recurso (imagem, ícone etc.).

    • Em desenvolvimento normal  : usa a pasta do próprio script.
    • Compilado com PyInstaller  : usa sys._MEIPASS (pasta temporária onde
      o executável --onefile extrai os arquivos embutidos em tempo de execução).
    """
...
class PDFManagerApp(QMainWindow):
    ...
    # ------------------------------------------------------------------
    # 🔧 Utilitário: instala pacote Python automaticamente via pip
    # ------------------------------------------------------------------
    def _ensure_package(self, import_name, install_name=None, extra_info=""):
        """
        Verifica se um pacote está disponível e, se não estiver,
        oferece instalação automática via pip — sem precisar de nada externo.
        """
        import importlib
        try:
            ...
            msg = (f"O pacote  '{pkg}'  não está instalado.\n"
                   f"Ele é necessário para esta função.")
            if extra_info:
                msg += f"\n\n{extra_info}"
            msg += "\n\nDeseja instalar agora?\n(pode levar alguns minutos na primeira vez)"

            reply = QMessageBox.question(self, "Instalar Dependência", msg,
                                         QMessageBox.Yes | QMessageBox.No)
            if reply != QMessageBox.Yes:
                return False

            prog = QProgressDialog(
                f"Instalando  {pkg} ...\nAguarde, não feche a janela.", None, 0, 0, self
            )
            prog.setWindowTitle("Instalando dependência")
            ...
            try:
                ...
                if result.returncode == 0:
                    QMessageBox.information(
                        self, "Instalado com sucesso",
                        f"'{pkg}' foi instalado!\n\n"
                        "Clique OK e tente a operação novamente."
                    )
                    # Recarrega o sys.path para que o import funcione sem reiniciar
                    import importlib
                    importlib.invalidate_caches()
                    return True
                else:
                    QMessageBox.critical(
                        self, "Falha na instalação",
    ...
    # ------------------------------------------------------------------
    # ✅ Fazer OCR nas páginas selecionadas (EasyOCR – 100% Python, sem Tesseract)
    # ------------------------------------------------------------------
    def apply_ocr(self):
        selected = self.get_selected_pages_from_thumbs()
        if not selected:
        ...
        # --- Auto-instala easyocr (puro Python, sem binário externo) ---
        if not self._ensure_package(
            "easyocr", "easyocr",
            "EasyOCR é uma biblioteca de OCR 100% Python.\n"
            "• Não exige Tesseract nem nenhum programa instalado no sistema.\n"
            "• Na PRIMEIRA execução baixará os modelos (~200 MB) automaticamente.\n"
            "• Nas próximas vezes os modelos já estarão em cache."
        ):
            return

        # --- numpy é necessário para converter pixels → array ---
        if not self._ensure_package("numpy", "numpy"):
            return

        # --- Importa após garantir instalação ---
        try:
            import importlib
            importlib.invalidate_caches()
            import easyocr       # noqa
        ...
        lang_map = {
            ...
            "Alemão":               ["de"],
            "Italiano":             ["it"],
            "Chinês Simplificado":  ["ch_sim"],
            "Japonês":              ["ja"],
        }
        lang_label, ok = QInputDialog.getItem(
            self, "Idioma do OCR",
            "Escolha o idioma principal do documento:",
            list(lang_map.keys()), 0, False
        )
        if not ok:
            return
        langs = lang_map[lang_label]

        out_path, _ = QFileDialog.getSaveFileName(
            self, "Salvar PDF Pesquisável (OCR)", "", "PDF Files (*.pdf)"
        )
        if not out_path:
            return
        ...
        progress = QProgressDialog(
            ...
            "(1ª vez: aguarde o download dos modelos ~200 MB)",
            "Cancelar", 0, len(selected) + 1, self
        )
        progress.setWindowTitle("OCR em andamento – aguarde")
        progress.setWindowModality(Qt.WindowModal)
        ...
        new_doc = fitz.open()
        try:
            # Inicializa o reader (baixa modelos na 1ª vez; usa cache depois)
            ...
            for i, (path, page_num) in enumerate(selected):
                ...
                progress.setLabelText(
                    f"OCR  –  pág. {page_num + 1}  |  {os.path.basename(path)}"
                )
                QApplication.processEvents()
                ...
                # Renderiza a página em 200 DPI para melhor precisão do OCR
                scale = 200 / 72
                mat = fitz.Matrix(scale, scale)
                pix = src_page.get_pixmap(matrix=mat, alpha=False)

                # Converte bytes do pixmap → numpy array RGB
                ...
                # Cria página de saída com dimensões originais
                new_page = new_doc.new_page(
                    width=page_rect.width, height=page_rect.height
                )
                # Cola a imagem original da página (mantém visual intacto)
                new_page.show_pdf_page(new_page.rect, src_doc, page_num)

                # Fatores de escala: pixel-imagem → ponto-PDF
                sx = page_rect.width  / pix.width
                sy = page_rect.height / pix.height

                # Insere camada de texto invisível e pesquisável
                for (bbox, text, confidence) in ocr_results:
                    ...
                    font_size = max(4.0, (y2 - y1) * 0.85)

                    # render_mode=3 → texto invisível (padrão PDF/A para OCR)
                    try:
                        ...
                        # Fallback: texto branco (visualmente invisível sobre fundo branco)
                        new_page.insert_text(
                            fitz.Point(x1, y2 - 1),
                            text,
                            fontsize=font_size,
            ...
            if not progress.wasCanceled():
                ...
                QMessageBox.information(
                    ...
                    f"Arquivo:  {out_path}\n"
                    f"Páginas:  {count}\n"
                    f"Idioma(s): {', '.join(langs)}\n\n"
                    "Agora você pode usar Ctrl+F para pesquisar texto no PDF."
                )
            else:
                new_doc.close()
                progress.close()
